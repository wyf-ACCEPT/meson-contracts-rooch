...
module Meson::MesonSwap {
    ...
    public entry fun postSwap<CoinType: key + store>(
        sender: &signer,
    ...
    // Named consistently with solidity contracts
    public entry fun bondSwap<CoinType: key + store>(
        sender: &signer,
        encoded_swap: vector<u8>,
        pool_index: u64,
    ) {
    ...
    public entry fun cancelSwap<CoinType: key + store>(_sender: &signer, encoded_swap: vector<u8>) {
        ...
        account_coin_store::deposit<CoinType>(from_address, coins);
    }


    // Named consistently with solidity contracts
    public entry fun executeSwap<CoinType: key + store>(
        ...
        if (deposit_to_pool) {
            ...
            account_coin_store::deposit<CoinType>(MesonStates::owner_of_pool(pool_index), coins);
        }
    }
}